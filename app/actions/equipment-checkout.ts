...
export async function createEquipmentOrderCheckout(
  ...
  // Validate all products have depositPercent configured
        ...
        create: cartItems.map((item) => {
          ...
          return {
            ...
            estimatedShipDate: product.leadTimeDays
              ? new Date(
                  Date.now() +
                    product.leadTimeDays * 24 * 60 * 60 * 1000
                )
              : undefined,
          };
        }),
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { orderId: order.id, paymentType: "deposit" },
...
export async function createBalanceCheckoutForItem(orderItemId: string) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { orderItemId: item.id, paymentType: "balance" },
